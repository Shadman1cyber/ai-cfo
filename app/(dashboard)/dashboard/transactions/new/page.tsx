...
export default function TransactionFormPage() {
  ...
  useEffect(() => {
    if (formData.description && formData.amount && !formData.categoryId && !isEdit) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEdit ? `/api/transactions/${params.id}` : "/api/transactions";
      ...
      if (!res.ok) {
        if (json.details) {
          ...
          setErrors({ amount: json.error || "خطا در ذخیره" });
        }
        return;
      }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {isEdit ? "اطلاعات تراکنش را به‌روزرسانی کنید" : "تراکنش مالی جدید ثبت کنید"}
          </p>
        </div>
        <Link
      ...
      <form onSubmit={handleSubmit} className="bg-card border rounded-xl p-6 space-y-6" noValidate>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-border">
          ...
          >
            {submitting ? "در حال ذخیره..." : isEdit ? "به‌روزرسانی" : "ثبت تراکنش"}
          </button>
          <Link
            href="/dashboard/transactions"
